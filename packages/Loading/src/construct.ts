...
export default class LoadingConstruct extends Loading {
  fullscreen = true;
  target: HTMLBodyElement | HTMLElement | null = null;
  visible = false;
  lock = false;

  vm: Loading | null = null;

  onScroll: null | (() => void) = null;
  ...
  close () {
    ...
    this.$once('after-leave', () => {
      ...
      if (!this.fullscreen) {
        if (this.vm?.$el) {
          (this.vm.$el as HTMLElement).style.marginTop = '';
        }
        if (!this.lock) {
